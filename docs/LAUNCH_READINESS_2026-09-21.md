...
| N1  | Navigation  | "Work with ODD", the B2B gateway every commercial journey routes through, sits two clicks deep behind a label reading "Info".                                                                                                       | **Reverted 2026-09-22 on Ronny's instruction.** It was promoted to a top-level nav item on 2026-09-21 and is now back inside the Info dropdown, first of four. The measurement that supported promoting it still holds — five items fit on one row at every width where the desktop nav exists, 83 px spare at the narrowest, 1040 px — so this is a brand decision, not a layout constraint. The dropdown test now asserts the Work with ODD route too, which it did not before. | Nav probe at 1040–2560 px; `interactions.spec.ts`                         | Ronny — settled                                        | NOT REQUIRED                                           |
